...
const Kienyenjimenu = () => {
  ...
  return (
    <div className="dark:bg-dark-default">
      ...
      <h1 className="text-3xl font-bold text-center mb-6 dark:text-gray-100">Kienyenji Menu 🍽️</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        {menuItems.map((item) => (
          <div
            key={item.id}
            className="border p-4 rounded-lg shadow-lg text-center cursor-pointer  dark:border-orange-400 dark:text-gray-100 "
            onClick={() => handleItemClick(item)}
          >
            <img src={item.image} alt={item.name} className="w-full h-40 object-cover rounded-md mb-4" />
            <h2 className="text-xl font-semibold">{item.name}</h2>
            <p className="text-lg font-bold text-orange-500">Ksh {item.price.toFixed(2)}</p>
          </div>
        ))}
      </div>
    </div>
